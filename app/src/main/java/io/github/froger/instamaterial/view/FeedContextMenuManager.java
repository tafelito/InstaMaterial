...
public void toggleContextMenuFromView(View openingView, int feedItem, FeedContextMenu.OnFeedContextMenuItemClickListener listener) {
    ...
    private void setupContextMenuInitialPosition(View openingView) {
        ...
        int additionalMargin = Utils.dpToPx(16);
        WindowManager wm = (WindowManager) contextMenuView.getContext().getSystemService(Context.WINDOW_SERVICE);
        Display display = wm.getDefaultDisplay();
        Point size = new Point();
        display.getSize(size);
        contextMenuView.setTranslationX(size.x - contextMenuView.getWidth());
        contextMenuView.setTranslationY(openingViewLocation[1] - contextMenuView.getHeight() - additionalMargin);
    }
